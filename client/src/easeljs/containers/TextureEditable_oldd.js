...
export default class TextureEditable extends createjs.Bitmap{
  ...
  constructor(imageOrUri, instructions){
    console.log("constructor");
      ...
      this.x =  - MainStage.currentStage.x + MainStage.currentStage.regX;
      this.y = 0;

      this.customMask = new createjs.Shape();
      this.customMask.graphics.setStrokeStyle(1);
      console.log(`x: ${MainStage.currentStage.x}`);
      console.log(`regX: ${MainStage.currentStage.regX}`);
  ...
  drawTexture(e){
    ...
    const decalage = - MainStage.currentStage.x + MainStage.currentStage.regX;
    if(instructionLength > 1){
      let closePoint = this.customMask.graphics._activeInstructions.pop();
      this.customMask.graphics.lineTo(e.stageX+decalage, e.stageY);
      this.customMask.graphics._activeInstructions.push(closePoint);
    }
    else{
      this.customMask.graphics.moveTo(e.stageX+decalage, e.stageY);
      this.customMask.graphics.lineTo(e.stageX+decalage, e.stageY);
      TextureEditable.parent.addChild(this);
      this.mask = this.customMask;
    }
